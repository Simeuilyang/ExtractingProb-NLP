...
public class FileController {
   ...
   private final String PATH = "E:\\심의량\\공개 소프트웨어 개발자 대회\\ExtractingProb-NLP\\extractingNLP\\src\\main\\webapp\\resources\\file\\";
...
//      }
   ...
   public boolean deleteFile(@PathVariable("fid") int fid) throws SQLException {
    ...
    //로컬 파일삭제 
      String filename = fDao.getFnamebyfid(fid);
      System.out.println("filename: " + filename);
      filename = filename.replace(".pdf", "");
      File file[] = new File[5];
      file[0] = new File(PATH+fid+"_"+filename+".pdf");
      file[1] = new File(PATH+fid+"_"+filename+"_out.txt");
      file[2] = new File(PATH+fid+"_"+filename+"_out.xml");
      file[3] = new File(PATH+"\\questions\\"+fid+"_"+filename+"_keword.txt");
      file[4] = new File(PATH+"\\questions\\"+fid+"_"+filename+"_sentence.txt");
   ...
   @ResponseBody
   @GetMapping("/file/move/{fid}/{movdid}/{aid}")
   public boolean moveFile(@PathVariable("fid") int fid, @PathVariable("movdid") int movdid, @PathVariable("aid") String aid) throws SQLException {
      System.out.println("MOVE FILE!! fid:" + fid + " did:" + movdid);
      return fDao.moveFile(fid, movdid, aid);
   }
   ...
   public ModelAndView upload(MultipartHttpServletRequest request, @PathVariable("fid") String fid) throws Exception{
	   ...
	   if(itr.hasNext()) {
		   ...
		   for(int i=0;i<mpf.size();i++) {
			   String origName = new String(((MultipartFile) mpf.get(i)).getOriginalFilename());
